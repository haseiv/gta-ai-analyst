...
class StatusCog(commands.Cog):
    ...
    @app_commands.command(name="status", description="Показать статус анализа")
    @app_commands.describe(analysis_id="Номер анализа, например A184")
    async def status(self, interaction: discord.Interaction, analysis_id: str) -> None:
        ...
        if job is None and row is None:
            await interaction.response.send_message(f"Анализ #{analysis_id} не найден.", ephemeral=True)
            return
        status = job.status if job else row.status
        progress = job.progress if job else row.progress
        if status == "FAILED":
            await interaction.response.send_message(
                embed=build_failed_embed(analysis_id, row.error if row and row.error else "Неизвестная ошибка")
            )
            return
